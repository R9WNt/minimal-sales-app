    ...
    <>
        {Array.from({ length: 8 }).map((_, i) => {
            ...
            onClick={(e: React.MouseEvent<HTMLButtonElement>) => {
                e.stopPropagation();
                console.log("FAQRing: button click", i, "selectedBefore", selected);
                if (selected === i) {
                    setAllowDescription(true);
                    // open/close immediately for debug
                    setShowDescription((s) => !s);
                    console.log("FAQRing: toggled showDescription (debug)");
                } else {
                    setSelected(i);
                    setAllowDescription(true);
                    setShowDescription(true);
                    console.log("FAQRing: setSelected + opened description (debug)");
                }
            }}
